...
class OnOffBlock : public ActorBlockBase {
...
Profile* OnOffBlock::cProfile = blox::getRegistrar()->newProfile<OnOffBlock>("oos")
...
void OnOffBlock::spawnItemUp() {
    zPosOffset = 0.0f;
    changeState(StateID_Wait);
}

void OnOffBlock::spawnItemDown() {
    zPosOffset = 0.0f;
    changeState(StateID_Wait);
}

bool OnOffBlock::isBlockActive() {
    return true;
}
